...
const blake3 = require('@noble/hashes/blake3'); // Removed blake2b instances and replaced with blake3 in sighash.js and package.json
...
const blake3_256 = (buf, key) => {
  return Buffer.from(blake3.blake3(buf, {key}));
};
...
function HashWriter(){
	...
	//let me = this;
	this.hash = {
		hash: blake3.create({key: TransactionSigningHashKey}),
		bw:new BufferWriter(),
		update: function(buf){
			/*
			if(me.L){
...
var sighash2 = function sighash(transaction, sighashType, inputNumber, subscript, satoshisBN, flags) {
	...
	let buf = new BufferWriter()
		...
		.toBuffer()
	let ret = blake3_256(buf, TransactionSigningHashKey);
